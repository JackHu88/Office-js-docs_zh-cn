...
获取或设置邮件的密件抄送 (Bcc) 行上的收件人。 仅限撰写模式。
...
将 `end` 属性表示为协调世界时 (UTC) 的日期和时间值。 可以使用 [`convertToLocalClientTime`](Office.context.mailbox.md#converttolocalclienttimetimevalue--localclienttime) 方法将 end 属性值转换为客户端的本地日期和时间。
...
使用 [`Time.setAsync`](Time.md#setasyncdatetime-options-callback) 方法设置结束时间时，应使用 [`convertToUtcClientTime`](Office.context.mailbox.md#converttoutcclienttimeinput--date) 方法将客户端的本地时间转换为服务器的 UTC。
...
以下示例通过使用 `Time` 对象的 [`setAsync`](Time.md#setasyncdatetime-options-callback) 方法，设置撰写模式下约会的结束时间。
...
`from` 和 [`sender`](Office.context.mailbox.item.md#sender-emailaddressdetails) 属性表示同一个人，邮件由代理人发送的除外。 在此情况下，`from` 属性表示代理程序，而 sender 属性表示代理人。
...
在撰写模式下，对于尚未保存到服务器的项目，`itemId` 属性返回 `null`。 如果项目标识符是必需的，[`saveAsync`](Office.context.mailbox.item.md#saveasyncoptions-callback) 方法可用于将项目保存到服务器，这将在回调函数的 [`AsyncResult.value`](simple-types.md#asyncresult) 参数中返回项目标识符。
...
以下代码检查项目标识符是否存在。 如果 `itemId` 属性返回 `null` 或 `undefined`，则将项目保存到服务器，并从异步结果中获取项目标识符。
...
normalizedSubject 属性获取包含由电子邮件程序添加的任何标准前缀（如 `RE:` 和 `FW:`）的项目主题。 若要获取包含完整前缀的项目主题，请使用 [`subject`](Office.context.mailbox.item.md#subject-stringsubject) 属性。
...
[`from`](Office.context.mailbox.item.md#from-emailaddressdetails) 和 `sender` 属性表示同一个人，邮件由代理人发送的除外。 在此情况下，`from` 属性表示代理程序，而 sender 属性表示代理人。
...
将 `start` 属性表示为协调世界时 (UTC) 的日期和时间值。 可以使用 [`convertToLocalClientTime`](Office.context.mailbox.md#converttolocalclienttimetimevalue--localclienttime) 方法将该值转换为客户端的本地日期和时间。
...
使用 [`Time.setAsync`](Time.md#setasyncdatetime-options-callback) 方法设置开始时间时，应使用 [`convertToUtcClientTime`](Office.context.mailbox.md#converttoutcclienttimeinput--date) 方法将客户端的本地时间转换为服务器的 UTC。
...
以下示例通过使用 `Time` 对象的 [`setAsync`](Time.md#setasyncdatetime-options-callback) 方法，设置撰写模式下约会的开始时间。
...
`addItemAttachmentAsync` 方法将包含指定 Exchange 标识符的项目附加到撰写窗体中的项目。 如果你指定一个回调方法，此方法使用一个 `asyncResult` 参数调用，该参数包含一个附件标识符或代码，指示附加项目过程中出现的任何错误。 你可以使用 `options` 参数将状态信息传递给回调方法（如果需要）。
...
如果在项目的正文属性上指定 `ItemHasRegularExpressionMatch` 规则，则正则表达式应进一步筛选正文并应尝试返回该项目的整个正文。 使用正则表达式（如 `.*`）获取项目的整个正文并不总是返回预期的结果。 而是使用 [`Body.getAsync`](Body.md#getasynccoerciontype-options-callback) 方法检索整个正文。
...
调用时，此方法将当前邮件保存为草稿，并通过回调方法返回项目 ID。
...
如果成功，该项目标识符在 `asyncResult.value` 属性中提供。
...
|要求| 值|
|---|---|
|[最低版本的邮箱要求集](./tutorial-api-requirement-sets.md)| 1.2|
|[最低权限级别](../../docs/outlook/understanding-outlook-add-in-permissions.md)| ReadWriteItem|
|适用的 Outlook 模式| 撰写|

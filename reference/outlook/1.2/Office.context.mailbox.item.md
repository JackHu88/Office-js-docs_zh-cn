...
在撰写模式下，对于尚未保存到服务器的项目，`itemId` 属性返回 `null`。 如果项目标识符是必需的，[`saveAsync`](Office.context.mailbox.item.md#saveAsync) 方法可用于将项目保存到服务器，这将在回调函数的 [`AsyncResult.value`](simple-types.md#asyncresult) 参数中返回项目标识符。
...
如果在项目的正文属性上指定 `ItemHasRegularExpressionMatch` 规则，则正则表达式应进一步筛选正文并应尝试返回该项目的整个正文。 使用正则表达式（如 `.*`）获取项目的整个正文并不总是返回预期的结果。 而是使用 [`Body.getAsync`](Body.md#getAsync) 方法检索整个正文。

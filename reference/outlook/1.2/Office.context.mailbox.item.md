...
在撰写模式下，对于尚未保存到存储的项目，`itemId` 属性返回 `null`。如果项目标识符是必需的，[`saveAsync`](Office.context.mailbox.item.md#saveAsync) 方法可用于将项目保存到存储，这将在回调函数的 [`AsyncResult.value`](simple-types.md#asyncresult) 参数中返回项目标识符。

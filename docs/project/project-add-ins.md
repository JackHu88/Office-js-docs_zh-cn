...
在文件共享中可以有多个外接程序清单 XML 文件和子目录。 你可以通过使用 Project 2013 中的“**信任中心**“对话框中“**受信任的外接程序目录**”选项卡来添加或删除清单目录位置和目录。 若要显示 Project 中的外接程序，清单中的 **SourceLocation** 元素必须指向现有网站或 HTML 源文件。
...
6. 在“**受信任的外接程序目录**”窗格（请参阅图 1）中的“**目录 URL**”文本框中添加 `\\ServerName\AppManifests` 路径，选择“**添加目录**”，然后选择“**确定**”。
     ...
     >**注意**  图 1 显示“**受信任的目录地址**”列表中专用目录的两个文件共享和一个假设 URL。 只有一个文件共享可以成为默认文件共享，并且只有一个目录 URL 可以成为默认目录。 例如，如果你将 `\\Server2\AppManifests` 设置为默认，Project 会清除 `\\ServerName\AppManifests` 的“**默认**”复选框。如果你更改默认选择，你可以选择“**清除**”来删除已安装的外接程序，然后重启 Project。 如果你在 Project 处于打开状态时将外接程序添加到默认文件共享或 SharePoint 目录，则应该重启 Project。

    **图 1. 使用信任中心添加外接程序清单的目录**

    ![使用信任中心添加应用部件清单 (manifest)](../../images/pj15_AgaveOverview_TrustCenter.PNG)
...
- [创建将 REST 与本地 Project Server OData 服务结合使用的 Project 加载项](../project/create-a-project-add-in-that-uses-rest-with-an-on-premises-odata-service.md)

...
# 在 Microsoft Azure 上托管 Office 外接程序
...
- 如何向 Office 外接程序发布并将其承载于 Azure 网站上
...
## 使用 .NET 的 Azure SDK、Azure 订阅和 Office 2013 来设置开发计算机
...
3. 获取 Azure 帐户。
     ...
     >**注意：**如果你是 MSDN 订户，[你可以获取 Azure 订阅作为你的 MSDN 订阅的一部分](http://www.windowsazure.com/en-us/pricing/member-offers/msdn-benefits/)。 如果你不是 MSDN 订户，你仍可以[在 Microsoft Azure 网站获取免费试用版 Azure](https://azure.microsoft.com/en-us/pricing/free-trial/)。 
...
## 步骤 2：将文件共享添加到受信任的加载项目录，使 Office 客户端应用程序信任安装 Office 外接程序的位置
...
5. 选中“**在菜单中显示**”的复选框。 将外接程序 XML 清单文件存储在受信任的外接程序目录的共享中时，外接程序将显示在“**Office 外接程序**”对话框中的“**共享文件夹**”下。
...
1. 在 Visual Studio 的“**视图**”菜单中，选择“**服务器资源管理器**”。 右键单击 **Azure** 并选择“**连接到 Microsoft Azure 订阅**”。 请按说明连接到你的 Azure 订阅。
...
6. 在 Visual Studio 2015 中，右键单击“**服务器资源管理器**”中的新建网站，选择“**下载发布配置文件**”，然后将配置文件保存至你的计算机。 发布配置文件包含你的凭据，使你可以执行 [步骤 5：将你的 Office 外接程序发布到 Azure 网站](../publish/host-an-office-add-in-on-microsoft-azure.md#step-5-publish-your-office-add-in-to-the-azure-website)。
...
5. 选择“**创建 WEB 应用**”。 Azure 管理门户将创建网站并重定向到“**网站**”页，你可以在此页上查看网站的状态。
    
    当网站处于“**运行**”状态时，请在“**名称**”列下选择网站的 URL。 你的浏览器会打开并显示带有“**你的 Web 应用已创建!**”消息的网页。 
...
## 步骤 4：在 Visual Studio 中创建 Office 外接程序
...
8. Visual Studio 完成发布 Web 项目后，浏览器将打开并显示网页，其中显示“已成功创建此 Web 应用”文本。这是网站当前的默认页。
    ...
    若要查看你的外接程序的网页，请将 URL 更改为使用 https: 并添加你的外接程序的默认 HTML 页的路径。 例如，已更改的 URL 应类似于 https://YourDomain.azurewebsites.net/Addin/Home/Home.html。 这可确认你的外接程序的网站现在托管于 Azure 上。 复制此 URL，因为你稍后在本主题编辑外接程序清单文件时将需要此 URL。
    

## 步骤 6：编辑外接程序清单文件以指向 Azure 上的 Office 外接程序
...
## 步骤 7：在 Office 客户端应用程序中插入并运行加载项

...
* 将解决方案应用于常见的客户方案。
* 应用设计最佳实践。
* 纳入“[Office UI 结构](https://dev.office.com/fabric/getting-started/)”组件和样式。
...
3. 复制并修改 [UX 设计模式](https://github.com/OfficeDev/Office-Add-in-UX-Design-Patterns/tree/master/Patterns)以满足你的特定需求。
...
* **试用版功能** - 提醒用户尝试使用的功能在外接程序试用版中不可用。 或者，如果你的外接程序是免费的，但是其中的一项功能需要订阅，则应考虑使用此模式。 你也可以在试用结束后使用此模式以提供降级体验。
...
* **导航（具有命令）** - 显示汉堡菜单，在任务窗格中具有命令（或操作）按钮。 当你想要一起提供站点导航和命令选项时，请使用此模式。  
...
* **消息横幅** - 在可折叠为一行、扩展到多行或解除的横幅中提供信息和简单调用操作（可选）。 使用消息横幅来在外接程序启动时报告服务更新或有用的提示。 

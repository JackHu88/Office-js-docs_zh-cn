...
使用 _cellFormat_ 可选参数更改单元格格式值，例如宽度、高度、字体、背景、对齐等。传递作为 _cellFormat_ 参数的值是一个数组，其中包含指定目标单元格和要对单元格应用的格式的 JavaScript 对象列表。例如：
...
## <a name="supported-formatting-keys-and-values"></a>支持的格式键和值
...
|**键**|**值**|**注释**|
|:-----|:-----|:-----|
| `backgroundColor:`|"none" \| \<所有预定义的颜色名称\> \| #RRGGBB|预定义的颜色名称：<br/><br/>"black", "blue", "gray", "green", "orange", "pink", "purple", "red", "teal", "turquoise", "violet", "white", "yellow"|
...
| `fontStyle:`|“regular”\| "italic" \| "bold" \| “bold italic”|**注意**：此次发布时，将 `fontStyle:` 设置为“italic”，然后设置为“bold”（或相反）表现为这两种设置的结合。即，例如，如果先设置 “italic”，然后设置 “bold”，结果将为 “bold italic”。要_仅_在之前设置为 bold 或 italic 的范围上设置 italic 或 bold，必须先设置 `fontStyle:"regular"` 以清除之前的格式。|
| `fontSize:`|**Number**||
| `fontUnderlineStyle:`|"none" \| "single" \| "double" \| "single accounting" \| “double accounting”||
| `fontColor:`|“自动”\| \<所有预定义的颜色名称\> \| #RRGGBB||
| `fontDirection:`|“context”\| "left-to-right" \| “right-to-left”|Excel Online 当前不支持从右向左显示文本。然而，如果外接程序在 Excel Online 中运行时，将 `fontDirection:` 设置为“从右向左”，格式设置就保存在工作簿文件中，当在桌面 Desktop Excel 中打开工作簿文件时，即可正确显示。|
| `fontStrikethrough:`|**Boolean**||
| `fontSuperscript:`|**Boolean**||
| `fontSubScript:`|**Boolean**||
| `fontNormal:`|**Boolean**|将字体、字体样式、大小和效果设置为正常样式。这会将单元格字体格式化重置为默认值。相当于选中“**单元格格式**”对话框“**字体**”选项卡的“**正常字体**”复选框。|
...
|**键**|**值**|**注释**|
|:-----|:-----|:-----|
| `indentLeft:`|**Number**|与对齐值结合使用时，仅支持以下组合：<br/><br/><ul><li><code>alignHorizontal: "left"</code> 和 <code>indentLeft: \<value\></code></li></ul>|
| `indentRight:`|**Number**|与对齐值结合使用时，仅支持以下组合：<br/><br/><ul><li><code>alignHorizontal: "right"</code> 和 <code>indentRight: \<value\></code></li></ul>|
| `indentDistributed:`|**Number**|与对齐值结合使用时，仅支持以下组合：<br/><br/><ul><li><code>alignHorizontal: "distributed"</code> 和 <code>indentDistributed: \<value\></code></li></ul>|
